...
namespace Project._Project.Scripts.Runtime.GameCamera
{
    ...
    /// and scene-authored Cinemachine camera rigs.
    ///
    /// Priority:
    /// Activity Camera > retained Activity Camera for current Route > Route Camera > Default Camera.
    ///
    /// The real Unity Camera and CinemachineBrain stay operational-scene owned; Route and Activity
    /// content provide Cinemachine rigs only.
    /// </summary>
    ...
    public sealed class FirstGameCameraDirector : MonoBehaviour
    {
        ...
        private static void ApplyCinemachineRig(GameObject rig, FirstGameCameraAnchorHost anchors, int priority, string source)
        {
            ...
            {
                Debug.LogError($"[FIRSTGAME_CAMERA] Camera rig must not contain an AudioListener. Audio is owned by Immersive Audio. rig='{FormatRig(rig)}'.", rig);
                return;
            }
